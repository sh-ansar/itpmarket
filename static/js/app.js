(() => {
  ...
  function hasProductFilters(){return Boolean(($('#productSearch')?.value||'').trim()||$('#platformFilter')?.value||$('#brandFilter')?.value||$('#statusFilter')?.value||$('#freshnessFilter')?.value||state.products.scope!=='all');}
  function updateFilterResetVisibility(){const button=$('#resetFilters');if(button)button.hidden=!hasProductFilters();}
