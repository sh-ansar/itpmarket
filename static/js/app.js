(() => {
  ...
  const logSummaryHtml = task => {
    ...
    const rows=[
      ...
      [t('marketplace','Площадка'), esc(String(task?.metadata?.platform||'system').toUpperCase())],
  ...
  async function loadOverview(){
    try{
      ...
      $('#metricProducts').textContent=number(o.catalog_count);$('#metricPlatforms').textContent=`Kaspi ${number(o.kaspi_count)} · Ozon ${number(o.ozon_count)}`;$('#metricAnalyzed').textContent=`${dataCoverage.toFixed(1)}%`;$('#metricAnalyzedSub').textContent=`Kaspi ${number(o.kaspi_market_analyzed_count)}/${number(o.kaspi_count)} · Ozon ${number(o.ozon_data_ready_count)}/${number(o.ozon_count)}`;$('#metricRisks').textContent=number(o.risk_count);$('#metricOpportunity').textContent=money(o.price_potential_monthly_kzt ?? o.potential_margin_monthly_kzt);$('#metricOpportunitySub').textContent=Number(o.potential_position_count||0)>0?`${number(o.potential_position_count)} поз. · ${number(o.potential_units_total)} ед./мес.`:t('no_confirmed_opportunities','нет подтверждённых возможностей');
  ...
  function renderProducts(items){
    $('#productsBody').innerHTML=items.length?items.map(p=>{
      ...
      return `<tr><td><input class="row-check" type="checkbox" data-code="${esc(p.product_code)}" ${state.selected.has(p.product_code)?'checked':''}></td><td><div class="product-cell"><img src="${esc(p.image_url||'')}" onerror="this.style.visibility='hidden'"><div><b>${esc(p.title)}</b><small>${esc([productCodeText(p.source_product_code),p.brand,p.size].filter(Boolean).join(' · '))}</small></div></div></td><td><div class="cell-stack"><span class="badge ${p.platform}">${esc(p.platform_label)}</span>${sellerMeta}</div></td><td><div class="cell-stack price-stack"><span class="money">${money(price)}</span>${original}</div></td><td>${range}</td><td><div class="position-stack"><span class="${statusClass(p.status_tone)}">${esc(statusLabel(p.price_status))}</span>${p.price_rank?`<small class="rank-meta">${p.price_rank} / ${p.price_rank_total}</small>`:''}</div></td><td>${pot}</td><td><div class="updated-cell"><span>${dateText(p.updated_at)}</span></div></td><td><button class="open-row" data-open-product="${esc(p.product_code)}">${icon('chevron-right')}</button></td></tr>`;
  ...
  function renderDrawer(p){
    ...
    const offers=(p.offers||[]).map(o=>{const relation=p.platform==='ozon'?(o.match_method_label||t('exact_product','Точный товар')):(o.is_own?t('own_offer','Свой продавец'):t('same_card','Та же карточка'));const linkText=p.platform==='ozon'?t('open_ozon_card','Открыть карточку Ozon ↗'):t('open_kaspi_card','Открыть карточку Kaspi ↗');return `<div class="candidate"><div><b>${esc(o.merchant_name||t('seller','Продавец'))}</b><small>${money(o.price_kzt)}${o.merchant_rating?` · ${esc(t('rating','рейтинг'))} ${Number(o.merchant_rating).toFixed(1)}`:''}</small>${o.product_url?`<a href="${esc(o.product_url)}" target="_blank" rel="noreferrer">${esc(linkText)}</a>`:''}</div><span class="relation">${esc(relation)}</span></div>`}).join('')||`<div class="empty">${esc(t('offers_empty','Точные предложения конкурентов ещё не найдены'))}</div>`;
    const method=p.match_method_label?`<p><small>${esc(t('match_method','Метод сопоставления'))}</small><b>${esc(p.match_method_label)}</b></p>`:'';
    $('#drawerBody').innerHTML=`<div class="drawer-product"><img src="${esc(p.image_url||'')}" onerror="this.style.visibility='hidden'"><div><h3>${esc(p.title)}</h3><p><span class="badge ${esc(p.platform)}">${esc(p.platform_label)}</span> ${esc([productCodeText(p.source_product_code),p.brand,p.size].filter(Boolean).join(' · '))}</p><p><span class="${statusClass(p.status_tone)}">${esc(statusLabel(p.price_status))}</span></p>${method}</div></div>
      <div class="position-grid"><div class="position-card"><small>${esc(t('current_price_label','Текущая цена'))}</small><b>${money(current)}</b>${p.currency_original!=='KZT'?`<span>${number(p.price_original)} ${esc(p.currency_original)}</span>`:''}</div><div class="position-card"><small>${esc(t('seller_min','Минимальная цена'))}</small><b>${money(p.market_min_price_kzt)}</b>${lowLink}</div><div class="position-card"><small>${esc(t('seller_median','Средняя цена'))}</small><b>${money(p.market_median_price_kzt)}</b><span>${esc(t('offers_count','Предложений'))}: ${number(p.reference_count||0)}</span></div><div class="position-card"><small>${esc(t('seller_max','Максимальная цена'))}</small><b>${money(p.market_max_price_kzt)}</b>${highLink}</div></div>
      ...
      <section class="drawer-section"><h4>${esc(p.platform==='kaspi'?t('same_card_sellers','Продавцы этой же карточки'):t('ozon_market_offers','Рыночные предложения Ozon'))}</h4><div class="candidate-list">${offers}</div></section>`;
  }
