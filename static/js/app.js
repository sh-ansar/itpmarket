(() => {
  ...
  const state = {lang:'ru',theme:'system',page:'dashboard',overview:null,overviewLoading:false,products:{page:1,pages:1,pageSize:30,scope:'risks',items:[],requestStartedAt:0,lastDurationMs:0},selected:new Set(),tasks:[],tasksLoading:false,currentTask:null,currentProductCode:'',settings:null,catalogConfig:null,reportRequest:0,notifications:[],notificationsInitialized:false,lastNotificationId:0,inventoryLoaded:false,inventoryLoading:false,telegram:null};
...
${d.recovery_code}`);}catch(e){toast(e.message,true)}}
  ...
  function bind(){
    ...
    $('#resetFilters').onclick=()=>{$('#productSearch').value='';['platformFilter','brandFilter','statusFilter','freshnessFilter','productTypeFilter','sizeFilter','seasonFilter','characteristicGroupFilter'].forEach(id=>clearMultiSelect('#'+id));$$('[data-attribute-key]').forEach(node=>clearMultiSelect(node));state.products.scope='risks';$$('#scopeTabs button').forEach(b=>b.classList.toggle('active',b.dataset.scope==='risks'));loadCatalogConfiguration();updateFilterResetVisibility();loadProducts()};
