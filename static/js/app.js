...
async function stopProduct(code){ try{ const d=await api('/api/tasks/stop_by_product',{method:'POST',body:{product_code:code}}); if(d && d.stopped && d.stopped.length) toast(t('stopped_ok','Остановлено')) ; else if(d && d.stopped && d.stopped.length===0) toast(t('stopped_none','Связанные выполняемые операции не найдены'), true); if(typeof loadTasks==='function') loadTasks(); }catch(e){toast(e.message,true)} }
  ...
  function vatIncludedText(price,config={}){
    const enabled=Boolean(config?.vat_enabled);
    const rate=Number(config?.vat_rate||0);
    if(Number(price||0)<=0||!enabled||rate<=0)return '';
    return `\u0441 \u0443\u0447\u0451\u0442\u043e\u043c \u041d\u0414\u0421 ${number(rate)}% \u041a\u0430\u0437\u0430\u0445\u0441\u0442\u0430\u043d`;
  }

  function vatIncludedSuffix(price,config={}){
    const value=vatIncludedText(price,config);
    return value?` \u00b7 ${value}`:'';
  }
  function renderSubscriptionSettings(snapshot){
    ...
    const invoiceConfiguration=billing.invoice_configuration||{};
    ...
    const plans=planCatalog.map(plan=>
      `<option value="${esc(plan.code)}">${esc(plan.name)} \u00b7 ${number(plan.price_amount)} ${esc(plan.currency)} / ${billingPeriodLabel(plan)}${vatIncludedSuffix(plan.price_amount,invoiceConfiguration)}</option>`
    ).join('');
      ...
      billingHtml=`
        <section class="subscription-billing-card issued">
          ...
          <div class="subscription-invoice-grid">
            <article>
              <small>\u041f\u0430\u043a\u0435\u0442</small>
              <b>${esc(billingSub.plan_name||billingSub.plan_code||'Spyon')} ${billingPeriodLabel(planByCode(billingSub.plan_code))}</b>
            </article>
            ...
            <article>
              ...
              ${Number(invoice.vat_rate||0)>0
                ?`<small>\u0441 \u0443\u0447\u0451\u0442\u043e\u043c \u041d\u0414\u0421 ${number(invoice.vat_rate)}% \u041a\u0430\u0437\u0430\u0445\u0441\u0442\u0430\u043d</small>`
                :''
              }
            </article>
        ...
        <section class="subscription-billing-card">
          <div class="subscription-billing-head">
            <div>
              ...
              <span>
                ${money(unitPrice)} / ${billingPeriodLabel(planByCode(billingSub.plan_code))}${vatIncludedSuffix(unitPrice,invoiceConfiguration)}
              </span>
            </div>
    ...
    host.innerHTML=`
      <div class="subscription-summary ${active?'active':'pending'}">
        <div>
          ...
          <span>${active?`${number(current.price_amount)} ${esc(current.currency)} / ${billingPeriodLabel(planByCode(current.plan_code))}${vatIncludedSuffix(current.price_amount,invoiceConfiguration)} \u00b7 \u0434\u043e ${esc(String(current.ends_at||'\u2014'))}`:esc(ent.message||'\u041e\u0436\u0438\u0434\u0430\u0435\u0442\u0441\u044f \u043f\u043e\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0435\u043d\u0438\u0435')}</span>
        </div>
  ...
  function addonOrderHtml(order){
    ...
    const invoiceVatText=vatIncludedText(
      order.total_price,
      {
        vat_enabled:Number(invoice.vat_rate||0)>0,
  ...
  async function loadSettings(){
    try{
      ...
      if($('#tenantName')){
        ...
        $('#tenantLegalAddress').value=tenant.legal_address||'';$('#tenantActualAddress').value=tenant.actual_address||'';
      }
