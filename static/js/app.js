(() => {
  ...
  function renderPageHeading(){ const node=$('#pageTitle'); if(!node)return; const key=`${state.page}_page_title`; node.textContent=t(key,t(`nav_${state.page}`,'ITP Market Intelligence')); }
  ...
  function updateOperationActions(){ const platform=$('#operationPlatform').value;$('#operationAction').innerHTML=(ACTIONS[platform]||[]).filter(([id])=>id!=='backup_database'||user.role==='admin').map(([id,label])=>`<option value="${id}">${esc(label)}</option>`).join('');$('#operationScope').disabled=platform!=='kaspi';if(platform!=='kaspi')$('#operationScope').value='all'; }
