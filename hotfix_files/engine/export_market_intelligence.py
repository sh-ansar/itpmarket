...
def main() -> int:
    ...
    try:
        conn.execute(
            ...
            ("market_intelligence_html", report_scope if code_filter else "all", html_path.name, str(html_path), len(rows), args.user_id or None),
        )
        conn.execute(
            "INSERT INTO app_reports(report_type,scope,file_name,file_path,rows_count,created_by,created_at) VALUES(?,?,?,?,?,?,datetime('now'))",
            ("market_intelligence_csv", report_scope if code_filter else "all", csv_path.name, str(csv_path), len(rows), args.user_id or None),
        )
        conn.execute(
            "INSERT INTO app_reports(report_type,scope,file_name,file_path,rows_count,created_by,created_at) VALUES(?,?,?,?,?,?,datetime('now'))",
            ("market_intelligence_xlsx", report_scope if code_filter else "all", xlsx_path.name, str(xlsx_path), len(rows), args.user_id or None),
        )
        conn.commit()
    finally:
        conn.close()
