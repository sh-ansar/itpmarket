...
админку, запуск тестовой операции и получение уведомления. Если Telegram
включён, дополнительно проверьте `/status` в личном чате и тестовую доставку.
...
экземпляра задаётся `ITP_DISABLE_SCHEDULER=1`. Также нельзя держать два bot
worker с одним Telegram token: у проверочного экземпляра оставьте
`ITP_TELEGRAM_BOT_ENABLED=0`.
