...
Дополнения увеличивают лимит позиций выбранной площадки: +100 за 5 000 KZT, +500 за 18 000 KZT и +1 000 за 30 000 KZT. Поддерживаются счёт, безопасное переоформление неоплаченного счёта до загрузки платёжного документа, загрузка подтверждения и ручная проверка оплаты, снимок условий утверждённой подписки, одноразовый trial и напоминания об окончании подписки. Юридический профиль оператора для новых счетов закреплён в versioned source из утверждённой оферты; в platform settings сохраняются только параметры счёта (НДС, срок, префикс, услуга и основание). Каждый выданный счёт хранит независимый `seller_snapshot`, поэтому последующая утверждённая смена профиля не переписывает историю. При настроенном `SPYON_INVOICE_STAMP_SHA256` генерация PDF сверяет хеш внешней печати до создания файла. Схема self-service регистрации, счетов и подтверждений оплаты закреплена аддитивной PostgreSQL-миграцией `20260827_self_service_billing_v1.sql`, которая автоматически применяется при будущей production-поставке после backup. Лимиты каталога проверяются до замены существующего снимка, а суточный лимит операций резервируется атомарно.
...
- Price-actualisation actions are available only after the selected seller has at least one persisted marketplace catalogue membership. The guard is seller-scoped, returns HTTP 409 before quota reservation or task creation, and does not block catalogue collection or full synchronisation.
- Tenant operation history contains only structured `SPYON_PROGRESS` events and safe summaries. Raw technical log tails are restricted to superadmins and redact authorization, API key, client secret, password, token, and cookie values before returning them.
...
| Telegram-бот | Одноразовая привязка из Settings без передачи пароля, персональная доставка, retry/pause/logout | `telegram_bot.py`, `app.py` | Bot API и Settings status API/UI | Telegram Bot API, secret token в runtime env | `PASS` unit/API; `getMe` подтверждает настроенного бота |
...
версии и SHA-256: хеш вычисляется по точному versioned DOCX из `docs/legal` и
сверяется с закреплённым для этой версии digest. Изменённый DOCX не будет
использован для выдачи или акцепта без явного versioned изменения реестра.
