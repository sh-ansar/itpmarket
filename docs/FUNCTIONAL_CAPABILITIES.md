...
Подтверждено: 151 автоматический тест, Python compile, JavaScript syntax checks, PowerShell parse, согласованность Python-пакетов, read-only runtime diagnostic без `FAIL` и локальный HTTP 200 для `/health`, `/ready`, `/`, `/register`, `/api/public/plans` и новых static assets. Отдельно покрыты Telegram login/delivery/isolation, параллельные catalog reads, seller-selector cardinality, seller-scoped PostgreSQL routing Ozon.ru, точная source-bound привязка одного legacy-профиля и очереди товаров, повторное подключение к Chrome-профилю Ozon.kz, отмена устаревших UI-запросов, равные минимальные цены, tenant-scoped остатки, отсутствие двойного подсчёта, match confirm/reject, RBAC и direct API marketplace boundary.
