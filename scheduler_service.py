...
class SchedulerService:
    ...
    def run_due_once(self)->None:
        for schedule in self.saas.due_schedules():
            ...
            try:
                uid=int(schedule.get('created_by') or 0); task=self.task_manager.start(action,f"{info['label']} — по расписанию",self.command_builder(action,[],uid),info.get('resource') or [],metadata={'scope':'all','scheduled':True,'schedule_id':int(schedule['id']),'tenant_id':int(schedule['tenant_id']),'requested_by_id':uid,'platform':info.get('platform') or ('ozon' if action.startswith('ozon_') else 'kaspi')})
