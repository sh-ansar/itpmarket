...
def main() -> int:
    ...
    </style></head><body><header><h1>ITP Market Intelligence</h1><p>Сводный exact-only отчёт Kaspi и реестр Ozon · {datetime.now().strftime('%d.%m.%Y %H:%M')}</p></header><main><section class='metrics'><div class='card'><span>Позиций</span><b>{len(rows)}</b></div><div class='card'><span>Ценовых рисков</span><b>{risks}</b></div><div class='card'><span>Требуют проверки</span><b>{review}</b></div><div class='card'><span>Потенциал маржи / месяц</span><b>{money(opportunity)}</b></div></section><section class='panel'><table><thead><tr><th>Площадка</th><th>Товар</th><th>Текущая цена</th><th>Минимум</th><th>Медиана</th><th>Максимум</th><th>Позиция</th><th>Потенциал</th></tr></thead><tbody>{''.join(tr)}</tbody></table></section></main></body></html>"""
