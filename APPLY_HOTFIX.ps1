...
if (-not (Test-Path $SourceDir)) { throw "hotfix_files directory was not found: $SourceDir" }
if (-not (Test-Path $ManifestPath)) { throw "FILES_SHA256.txt was not found: $ManifestPath" }
...
foreach ($Line in Get-Content -LiteralPath $ManifestPath) {
    ...
    if (-not (Test-Path -LiteralPath $Source -PathType Leaf)) { throw "Package file is missing: $Relative" }
...
if ($ManifestEntries.Count -eq 0) { throw "The manifest does not contain files." }
...
$BackupDir = Join-Path $ProjectPath "backups\hotfix_3_4_15_$Stamp"
...
try {
    ...
    if ($Python) {
        ...
        try {
            & $Python -m compileall -q data_service.py collectors\ozon\ozon_validation_core.py SELF_TEST_HOTFIX_3_4_15.py
            if ($LASTEXITCODE -ne 0) { throw "Python compileall failed with exit code $LASTEXITCODE" }
    ...
    if ($Node) {
        & $Node.Source --check (Join-Path $ProjectPath "static\js\app.js")
...
Write-Host ""
Write-Host "Hotfix 3.4.15 installed successfully." -ForegroundColor Green
Write-Host "Backup: $BackupDir"
Write-Host "No database, runtime, browser profile, logs or environment files were deleted."
Write-Host "Run SELF_TEST_HOTFIX_3_4_15.bat and then START.bat."
